...
function observations = data_association(mu, sigma, observations, state_to_id_map)
	...
	for i=1:num_landmarks
		...
		%Sigma_z = to the jacobian x current covariance + constant noise

		Sigma_z = C*sigma*C'+eye(2,2)*0.01;
		Omega_z = inv(Sigma_z);
	...
	for i=1:dim_gated_associations
		...
		if(current_associations_value != min(A(:, proposed_landmark))) % NEGATIVE of the best friends condition
	...
	if(num_landmarks_measured > 1)
		...
		for i=1:dim_gated_associations
			...
			first_cond = second_row_min_value-associations(i,3);
			second_cond = second_col_min_value-associations(i,3);
			%check if the association is ambiguous
			if(first_cond<gamma_threshold || second_cond<gamma_threshold) %lonely best friend condition
				associations(i,2) = 0; %discard association, it is doubtful
			endif
