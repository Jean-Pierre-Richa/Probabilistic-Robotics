...
#set initial pose at the origin - we do not know the map and neither our location
mu = [0;  #x coordinate
      0;  #y coordinate
...
for t = 1:length(transitions)
  ...
  #EKF correct
  [mu, sigma] = correction(mu, sigma, observation, id_to_state_map, state_to_id_map);

  [mu, sigma, id_to_state_map, state_to_id_map] = addLandmarks(mu, sigma, observation, id_to_state_map, state_to_id_map);
